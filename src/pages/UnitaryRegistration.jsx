        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    ...
                    <div className="glass p-6 rounded-2xl border border-[var(--border-soft)]">
                        <label className="block text-sm font-bold text-[var(--text-muted)] mb-4 uppercase tracking-widest flex items-center gap-2">
                        ...
                        <div className="flex gap-2 mb-4">
                            <input 
                                ...
                                className="flex-1 bg-[var(--surface-glass)]/50 border border-[var(--border-soft)] rounded-xl px-4 py-3 text-[var(--text-primary)] focus:border-primary outline-none"
                            />
                            <button 
                                onClick={handleSearch}
                                disabled={searching}
                                className="bg-[var(--surface-glass)] hover:bg-slate-700 text-[var(--text-primary)] px-4 rounded-xl transition-all"
                            >
                            ...
                            <div className="max-h-60 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                                {products.map(p => (
                                    <div 
                                        ...
                                        className="p-3 bg-[var(--surface-glass)]/30 border border-[var(--border-soft)] rounded-lg cursor-pointer hover:border-primary/50 transition-all flex items-center gap-3"
                                    >
                                        <img src={p.images?.[0]?.src} className="w-10 h-10 rounded object-cover bg-[var(--surface-glass)]" alt="" />
                                        <div className="flex-1 overflow-hidden">
                                            <p className="text-sm font-bold text-[var(--text-primary)] truncate">{p.name.pt || p.name}</p>
                                            <p className="text-[10px] text-[var(--text-muted)] italic">SKU: {p.variants?.[0]?.sku || 'N/A'}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        {baseProduct && (
                            <div className="p-4 bg-primary/10 border border-primary/30 rounded-xl flex items-center gap-4 animate-in zoom-in-95 duration-300">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-[var(--text-primary)] font-bold">{baseProduct.name.pt || baseProduct.name}</p>
                                    <button 
                    ...
                    <div className="glass p-6 rounded-2xl border border-[var(--border-soft)]">
                        ...
                        <div 
                            className={`border-2 border-dashed rounded-2xl p-8 transition-all flex flex-col items-center justify-center gap-4 cursor-pointer
                                ${selectedFile ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-[var(--border-soft)] hover:border-primary/50 bg-[var(--surface-glass)]/30'}`}
                        ...
                        >
                            ...
                            {!selectedFile ? (
                                <>
                                    <div className="w-16 h-16 rounded-full bg-[var(--surface-glass)] flex items-center justify-center mb-2">
                                        <ImageIcon className="w-8 h-8 text-[var(--text-muted)]" />
                                    </div>
                                    <p className="text-[var(--text-muted)] text-sm text-center">Clique ou arraste a imagem aqui.<br/><span className="text-[10px] italic">O nome do arquivo será usado como SKU.</span></p>
                                </>
                            ) : (
                        ...
                        {selectedFile && (
                            <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                                ...
                                <p className="text-[var(--text-primary)] font-mono text-lg">{selectedFile.name.replace(/\.[^/.]+$/, "").replace(/[^a-zA-Z0-9-]/g, "")}</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div className="glass p-6 rounded-2xl border border-[var(--border-soft)] min-h-[400px] flex flex-col">
                        <label className="block text-sm font-bold text-[var(--text-muted)] mb-6 uppercase tracking-widest flex items-center gap-2">
                            Preview do Cadastro
                        </label>
                            ...
                            <div className="flex-1 flex flex-col h-full">
                                ...
                                <div className="mt-6 space-y-4">
                                    <div className="flex justify-between text-sm">
                                        <span className="text-[var(--text-muted)] italic">Replicação:</span>
                                        <span className="text-[var(--text-primary)] font-bold">{baseProduct ? 'Atributos, SEO e Categorias OK' : 'Aguardando base...'}</span>
                                    </div>
                                    <div className="flex justify-between text-sm">
                        ...
                        <div className="p-6 bg-emerald-500/20 border border-emerald-500/30 rounded-2xl animate-in fade-in zoom-in duration-500">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-10 h-10 rounded-full bg-emerald-500 flex items-center justify-center">
                                    <CheckCircle className="text-[var(--text-primary)] w-6 h-6" />
                                </div>
                                <div>
                            ...
                            <div className="flex items-center gap-4 bg-black/20 p-3 rounded-lg">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-[var(--text-primary)] text-sm font-bold truncate">{successData.name.pt}</p>
                                    <a 
