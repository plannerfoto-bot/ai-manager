        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-[#8A8F98] mt-2">Replicação inteligente de produtos com marca d'água automática.</p>
                </div>
                <div className="glass px-4 py-2 rounded-lg flex items-center gap-2 text-sm text-[#8A8F98]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    ...
                    <div className="glass p-6 rounded-2xl border border-white/10">
                        <label className="block text-sm font-bold text-[#8A8F98] mb-4 uppercase tracking-widest flex items-center gap-2">
                        ...
                        <div className="flex gap-2 mb-4">
                            <input 
                                ...
                                className="flex-1 bg-white/5/50 border border-white/10 rounded-xl px-4 py-3 text-[#EDEDEF] focus:border-primary outline-none"
                            />
                            <button 
                                onClick={handleSearch}
                                disabled={searching}
                                className="bg-white/5 hover:bg-slate-700 text-[#EDEDEF] px-4 rounded-xl transition-all"
                            >
                            ...
                            <div className="max-h-60 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                                {products.map(p => (
                                    <div 
                                        ...
                                        className="p-3 bg-white/5/30 border border-white/10 rounded-lg cursor-pointer hover:border-primary/50 transition-all flex items-center gap-3"
                                    >
                                        <img src={p.images?.[0]?.src} className="w-10 h-10 rounded object-cover bg-white/5" alt="" />
                                        <div className="flex-1 overflow-hidden">
                                            <p className="text-sm font-bold text-[#EDEDEF] truncate">{p.name.pt || p.name}</p>
                                            <p className="text-[10px] text-[#8A8F98] italic">SKU: {p.variants?.[0]?.sku || 'N/A'}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        {baseProduct && (
                            <div className="p-4 bg-primary/10 border border-primary/30 rounded-xl flex items-center gap-4 animate-in zoom-in-95 duration-300">
                                <img src={baseProduct.images?.[0]?.src} className="w-16 h-16 rounded-lg object-cover bg-white/5" alt="" />
                                <div className="flex-1">
                                    <p className="text-xs text-primary font-bold uppercase mb-1">Base Selecionada</p>
                                    <p className="text-[#EDEDEF] font-bold">{baseProduct.name.pt || baseProduct.name}</p>
                                    <button 
                    ...
                    <div className="glass p-6 rounded-2xl border border-white/10">
                        ...
                        <div 
                            className={`border-2 border-dashed rounded-2xl p-8 transition-all flex flex-col items-center justify-center gap-4 cursor-pointer
                                ${selectedFile ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-white/10 hover:border-primary/50 bg-white/5/30'}`}
                            ...
                            {!selectedFile ? (
                                <>
                                    <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-2">
                                        <ImageIcon className="w-8 h-8 text-[#8A8F98]" />
                                    </div>
                                    <p className="text-[#8A8F98] text-sm text-center">Clique ou arraste a imagem aqui.<br/><span className="text-[10px] italic">O nome do arquivo será usado como SKU.</span></p>
                                </>
                            ) : (
                                <div className="text-center">
                                    <p className="text-emerald-400 font-bold mb-2">Imagem Carregada!</p>
                                    <p className="text-[#EDEDEF] text-xs font-mono bg-slate-950 px-3 py-1 rounded-full border border-white/10">{selectedFile.name}</p>
                                </div>
                            )}
                        </div>
                        ...
                        {selectedFile && (
                            <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                                ...
                                <p className="text-[#EDEDEF] font-mono text-lg">{selectedFile.name.replace(/\.[^/.]+$/, "").replace(/[^a-zA-Z0-9-]/g, "")}</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div className="glass p-6 rounded-2xl border border-white/10 min-h-[400px] flex flex-col">
                        <label className="block text-sm font-bold text-[#8A8F98] mb-6 uppercase tracking-widest flex items-center gap-2">
                            Preview do Cadastro
                        </label>

                        {previewUrl ? (
                            <div className="flex-1 flex flex-col h-full">
                                <div className="relative rounded-xl overflow-hidden border border-white/10 group bg-slate-950 flex-1 flex items-center justify-center">
                                    <img src={previewUrl} className="max-h-[300px] object-contain" alt="Preview" />
                                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center text-[#EDEDEF] text-xs font-bold uppercase tracking-widest">
                                        Imagem Original
                                    </div>
                                </div>
                                <div className="mt-6 space-y-4">
                                    <div className="flex justify-between text-sm">
                                        <span className="text-[#8A8F98] italic">Replicação:</span>
                                        <span className="text-[#EDEDEF] font-bold">{baseProduct ? 'Atributos, SEO e Categorias OK' : 'Aguardando base...'}</span>
                                    </div>
                                    <div className="flex justify-between text-sm">
                                        <span className="text-[#8A8F98] italic">Marca d'água:</span>
                        ...
                        <button 
                            ...
                            className="mt-8 w-full py-5 bg-primary text-[#EDEDEF] font-bold rounded-2xl hover:shadow-[0_0_40px_rgba(37,99,235,0.4)] transition-all flex items-center justify-center gap-3 disabled:opacity-30 disabled:cursor-not-allowed group"
                        >
                        ...
                        <div className="p-6 bg-emerald-500/20 border border-emerald-500/30 rounded-2xl animate-in fade-in zoom-in duration-500">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-10 h-10 rounded-full bg-emerald-500 flex items-center justify-center">
                                    <CheckCircle className="text-[#EDEDEF] w-6 h-6" />
                                </div>
                                <div>
                                    <h3 className="text-[#EDEDEF] font-bold">Produto Criado!</h3>
                            ...
                            <div className="flex items-center gap-4 bg-black/20 p-3 rounded-lg">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-[#EDEDEF] text-sm font-bold truncate">{successData.name.pt}</p>
                                    <a 
