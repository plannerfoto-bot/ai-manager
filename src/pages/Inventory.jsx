...
const Inventory = ({ products = [], total = 0, page = 1, loading, onDelete, onRefresh, calculatorEnabled = false }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[var(--surface-glass)]/40 p-6 rounded-3xl border border-[var(--border-soft)]/50 backdrop-blur-xl">
        <div>
          <h1 className="text-3xl font-bold text-[var(--text-primary)] tracking-tight flex items-center gap-3">
            <Package className="text-primary w-8 h-8" />
            Produtos & Catálogo
          </h1>
          <p className="text-[var(--text-muted)] mt-1">Gerencie seu inventário e crie novos itens com agilidade.</p>
        </div>
      ...
      {viewMode === 'create' ? (
        ...
        <>
          {products.length === 0 && !loading ? (
            <div className="glass p-20 rounded-[40px] flex flex-col items-center justify-center text-center space-y-6 border-dashed border-2 border-[var(--border-soft)] bg-[var(--surface-glass)]/20">
              <div className="p-6 bg-[var(--surface-glass)]/50 rounded-full">
              ...
              <div className="lg:col-span-2 space-y-6">
                {loading ? (
                  <div className="flex flex-col items-center justify-center h-64 space-y-4">
                    ...
                    <p className="text-[var(--text-muted)]">Buscando catálogo...</p>
                  </div>
                ) : (
                  <>
                        ...
                        >
                          <div className="flex gap-4 text-left">
                            ...
                            <div className="flex-1 min-w-0">
                              <h3 className="text-[var(--text-primary)] font-bold truncate group-hover:text-primary transition-all">{getText(product.name)}</h3>
                              <p className="text-primary font-bold mt-1">
                                R$ {parseFloat(product.variants?.[0]?.price || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                              </p>
                              <div className="flex items-center gap-2 mt-2">
                                <span className="text-[10px] bg-[var(--surface-glass)] text-[var(--text-muted)] px-2 py-0.5 rounded-full uppercase tracking-wider">
                ...
                {selectedProduct ? (
                  <div className="glass p-6 rounded-2xl border-primary sticky top-8 animate-in fade-in slide-in-from-right-8 duration-500 shadow-2xl shadow-primary/10">
                    <div className="flex justify-between items-start mb-6">
                      <h2 className="text-xl font-bold text-[var(--text-primary)]">Ficha Técnica</h2>
                      <button 
                        onClick={() => setSelectedProduct(null)}
                        className="p-1 hover:bg-[var(--surface-glass)] rounded-full text-[var(--text-muted)]"
                      >
                        <X className="w-5 h-5" />
                      </button>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div className="aspect-video rounded-xl bg-[var(--surface-glass)] border border-[var(--border-soft)] overflow-hidden shadow-inner flex items-center justify-center">
                      ...
                      <div className="space-y-4">
                        <div className="p-4 rounded-xl bg-[var(--surface-glass)]/50 border border-[var(--border-soft)]">
                          <p className="text-xs text-[var(--text-muted)] uppercase font-bold mb-1 flex items-center gap-2">
                            <Tag className="w-3 h-3 text-primary" /> SEO & Tags
                          </p>
                          <p className="text-[var(--text-primary)] text-sm font-medium">{getText(selectedProduct.seo_title) || getText(selectedProduct.name)}</p>
                          <div className="flex flex-wrap gap-2 mt-2">
                            {getText(selectedProduct.tags) ? getText(selectedProduct.tags).split(',').map(tag => (
                              <span key={tag} className="text-[10px] btn-primary/10 text-[var(--accent)] px-2 py-0.5 rounded border border-indigo-500/20 font-bold uppercase">{tag.trim()}</span>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-3 rounded-xl bg-[var(--surface-glass)]/50 border border-[var(--border-soft)]">
                            <p className="text-[10px] text-[var(--text-muted)] uppercase font-bold mb-1 flex items-center gap-2">
                              <BarChart3 className="w-3 h-3" /> SKU
                            </p>
                            <p className="text-[var(--text-primary)] text-sm font-mono tracking-tighter truncate">{selectedProduct.variants?.[0]?.sku || 'AIOX-GEN'}</p>
                          </div>
                          <div className="p-3 rounded-xl bg-[var(--surface-glass)]/50 border border-[var(--border-soft)]">
                            <p className="text-[10px] text-[var(--text-muted)] uppercase font-bold mb-1 flex items-center gap-2">
                              <Layers className="w-3 h-3" /> Peso
                            </p>
                            <p className="text-[var(--text-primary)] text-sm font-bold">{selectedProduct.variants?.[0]?.weight || '0'} kg</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <button 
                          ...
                          className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-[var(--text-primary)] font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-pink-500/20 group"
                        >
                        ...
                        <button 
                          ...
                          className="w-full py-3 bg-rose-500/10 text-rose-500 font-bold rounded-xl hover:bg-rose-500 hover:text-[var(--text-primary)] transition-all flex items-center justify-center gap-2 border border-rose-500/20"
                        >
                  ...
                  <div className="glass p-12 rounded-2xl flex flex-col items-center justify-center text-center opacity-40 h-[400px] border-dashed border-2 border-[var(--border-soft)]">
                    <Package className="w-16 h-16 mb-4 text-slate-700" />
                    <p className="text-[var(--text-muted)] font-medium">Selecione um produto para visualizar os detalhes técnicos e SEO.</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {igModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-[var(--surface-glass)] border border-[var(--border-soft)] rounded-[28px] p-8 w-full max-w-lg shadow-2xl animate-in slide-in-from-bottom-8 duration-300">
            {/* Header */}
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-purple-600 to-pink-600 rounded-xl flex items-center justify-center">
                  <IgIcon className="w-5 h-5 text-[var(--text-primary)]" />
                </div>
                <div>
                  <h3 className="text-[var(--text-primary)] font-bold text-lg leading-tight">Publicar no Instagram</h3>
                  <p className="text-[var(--text-muted)] text-xs">{getText(selectedProduct?.name)}</p>
                </div>
              </div>
              <button 
                onClick={() => setIgModal(false)} 
                className="p-2 hover:bg-[var(--surface-glass)] rounded-xl text-[var(--text-muted)] transition-all"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex gap-4 mb-6">
              <label className={`flex items-center gap-3 p-3 flex-1 rounded-xl border cursor-pointer transition-all ${igPostFeed ? 'border-pink-500/40 bg-pink-500/10 text-pink-300' : 'border-[var(--border-soft)] text-[var(--text-muted)]'}`}>
                <input 
                  type="checkbox" 
                  checked={igPostFeed} 
              ...
              <label className={`flex items-center gap-3 p-3 flex-1 rounded-xl border cursor-pointer transition-all ${igPostStory ? 'border-purple-500/40 bg-purple-500/10 text-purple-300' : 'border-[var(--border-soft)] text-[var(--text-muted)]'}`}>
                <input 
                  type="checkbox" 
                  checked={igPostStory} 
            ...
            <div className="flex gap-3">
              <button 
                onClick={() => setIgModal(false)}
                className="flex-1 py-3 bg-[var(--surface-glass)] hover:bg-slate-700 text-[var(--text-primary)] font-bold rounded-xl transition-all"
              >
                Cancelar
              </button>
              <button 
                ...
                disabled={igLoading}
                className="flex-[2] py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 disabled:opacity-50 text-[var(--text-primary)] font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg"
              >
                {igLoading ? (
                  <>
