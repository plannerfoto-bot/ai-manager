  ...
  return (
    <div className="glass p-8 rounded-3xl border-primary max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-500">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-[var(--text-primary)]">Cadastro Manual em Lote</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-4">
          <label className="block text-sm font-bold text-[var(--text-muted)] uppercase tracking-wider mb-4">
            Imagens & Identificação ({items.length})
          </label>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            
            {items.map((item) => (
              <div key={item.id} className="relative rounded-2xl overflow-hidden border border-[var(--border-soft)] bg-[var(--surface-glass)]/50 group flex flex-col focus-within:border-primary focus-within:ring-1 focus-within:ring-primary/50 transition-all shadow-lg">
                <div className="aspect-square relative flex-shrink-0 bg-[var(--surface-glass)] border-b border-[var(--border-soft)]">
                  <img src={item.dataUrl} alt="Preview" className="w-full h-full object-cover" />
                  <button 
                    type="button"
                    onClick={() => removeItem(item.id)}
                    className="absolute top-3 right-3 p-2 bg-rose-500/90 hover:bg-rose-500 text-[var(--text-primary)] rounded-full transition-all shadow-lg"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                <div className="p-4 flex-1 flex flex-col">
                  <label className="text-xs text-[var(--text-muted)] font-bold uppercase mb-2">Nome deste produto</label>
                  <input
                    type="text"
                    required
                    value={item.name}
                    onChange={(e) => updateItemName(item.id, e.target.value)}
                    className="w-full bg-[var(--surface-glass)] border border-[var(--border-soft)] rounded-xl px-4 py-3 text-sm text-[var(--text-primary)] focus:border-primary outline-none transition-all"
        ...
        {!initialData && (
          <div className="pt-8 border-t border-[var(--border-soft)] space-y-6">
            <h3 className="text-lg font-bold text-[var(--text-primary)] mb-4">Informações Genéricas do Lote</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-bold text-[var(--text-muted)] uppercase tracking-wider">Preço Genérico (R$)</label>
                <input 
                  required
                  type="number" 
                  step="0.01"
                  ...
                  placeholder="0.00"
                  className="w-full bg-[var(--surface-glass)]/50 border border-[var(--border-soft)] rounded-xl px-4 py-3 text-[var(--text-primary)] focus:border-primary outline-none transition-all"
                />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-bold text-[var(--text-muted)] uppercase tracking-wider">Estoque Padrão</label>
                <input 
                  type="number" 
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-[var(--text-muted)] uppercase tracking-wider">Peso Padrão (kg)</label>
                <input 
                  type="number" 
                  step="0.1"
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-[var(--text-muted)] uppercase tracking-wider">Descrição Genérica (Opcional)</label>
              <textarea 
                rows="3"
                ...
                className="w-full bg-[var(--surface-glass)]/50 border border-[var(--border-soft)] rounded-xl px-4 py-3 text-[var(--text-primary)] focus:border-primary outline-none transition-all resize-none"
              />
            </div>
          </div>
        )}
